                ...
                <form class="yourform" action="{{ route('publisher.update', $publisher->id) }}" method="post"
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control @error('name') isinvalid @enderror" name="name"
                            value="{{ $publisher->name }}" required onkeypress='return harustext(event)'>
                        @error('name')
                    ...
                    <input type="submit" name="submit" class="btn btn-danger" value="Update" required onclick= "return confirm('Yakin ingin mengganti data ini?')">
                </form>
            </div>
        </div>
    </div>
